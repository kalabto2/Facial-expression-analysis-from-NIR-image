...
def show_dashboard(image_np_array, predictions, target=None, save_dashboard_fp=None):
    ...
    centerface_path = os.path.join(directory_path, "images", "VAmodel.png")

    circ_fp = str(centerface_path) # 'skeleton/FaceInference/images/VAmodel.png'
...
class CenterFace(object):
    def __init__(self, landmarks=True, verbose=True):
        ...
        file_path = os.path.abspath(__file__)
        directory_path = os.path.dirname(file_path)
        centerface_path = os.path.join(directory_path, "centerface.onnx")
        
        self.net = cv2.dnn.readNetFromONNX(str(centerface_path))
...
#         SPECTRUM_TRANSLATOR_CUSTOM_CYCLEGAN = "D"
    ...
    def _infer_instant(self, content, func):
        ...
        def _stacked_fns(_x1, _x2):
            ...
            if self.face_detector_model is not None:
                det_out = self.detect_faces(in1, in2)
                if det_out is None:
                    return None, None, None
                
                x = [f for f in det_out]
